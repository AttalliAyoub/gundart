...
class GunRefOptions {
  final GunRefOpt? opt;
  GunRefOptions({ this.opt });
  Map<String, dynamic> get json {
    return {'opt': opt?.json };
  }
}

typedef AckCallback = void Function({
  required Error? err,
  required dynamic ok,
});
...
// export interface IGunChainReference<DataType = Record<string, any>, ReferenceKey = any, IsTop extends 'pre_root' | 'root' | false = false> {
    ...
    IGunChainReference put(
      
      Map<String, dynamic> data,
      AckCallback? callback,
      GunRefOptions? options
      
      ) {
      // IGunChainReference<DataType, ReferenceKey, IsTop>
      return null as IGunChainReference;
    }
    ...
    */
    IGunChainReference get(
      ...
      //   paramB: Record<'off' | 'to' | 'next' | 'the' | 'on' | 'as' | 'back' | 'rid' | 'id', any> ) => void
        ) {
          return null as IGunChainReference;
        }
    ...
    */
    IGunChainReference opt(GunConstructorOptions options) {
      return null as IGunChainReference;
    }
    /**
     ...
     */
    IGunChainReference back(num? amount) {
      return null as IGunChainReference;
    }
     ...
     */
    IGunChainReference on(
      // callback: (data: DisallowPrimitives<IsTop, AlwaysDisallowedType<DataType>>, key: ReferenceKey) => void,
      OnCallback callback,
       bool? change
      // option?: {
      //   change: boolean;
     ...
     */
    IGunChainReference once(
            OnCallback callback,
       bool? change,
      // callback?: (data: (DisallowPrimitives<IsTop, AlwaysDisallowedType<Record<string, DataType>>>) | undefined, key: ReferenceKey) => void,
      // option?: {
      //   wait: number;
      // },
    ) {
    ...
    map(
      dynamic Function(dynamic value, String key)? callback,
      // callback?: (value: DataType, key: string) => DataType | undefined
      ) {
      return null as IGunChainReference;
     ...
     */
    IGunChainReference path(List<String> path) {
      return null as IGunChainReference;
    }
